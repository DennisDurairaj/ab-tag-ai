...
- [Agent Architecture Grilling](tickets/005-agent-architecture-grilling.md) — 9 tools defined (read_audio_metadata, search_open_library, search_audnexus, search_hardcover, write_id3_tags, write_ffmetadata, download_cover_art, move_file, dry_run_preview). Per-book sequential control flow. Provider fallback chain (Audnexus → Open Library → Hardcover) with manual review flag. Config file + CLI flags. TypeScript project structure.

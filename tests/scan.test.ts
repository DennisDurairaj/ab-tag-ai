...
describe("scanForAudioFiles", () => {
  it("returns empty array for non-existent directory", async () => {
    const result = await scanForAudioFiles("/nonexistent/path/12345");
    expect(result).toEqual([]);
  });

  it("finds MP3 files recursively", async () => {
    ...
    const result = await scanForAudioFiles(tmpDir);
  ...
  it("finds M4B files recursively", async () => {
    const tmpDir = makeTmpDir();
  ...
  it("infers metadata from filename when no ID3 tags present", async () => {
    const tmpDir = makeTmpDir();
  ...
  it("infers author and title from filename with dash separator", async () => {
    const tmpDir = makeTmpDir();
...
describe("detectMultiFileSets", () => {
  ...
  it("returns empty array when only single files in a directory", async () => {
    const tmpDir = makeTmpDir();
    touchFile(path.join(tmpDir, "book1.mp3"));

    const files = await scanForAudioFiles(tmpDir);
    expect(detectMultiFileSets(files)).toEqual([]);

    fs.rmSync(tmpDir, { recursive: true, force: true });
  });

  it("detects files sharing a common prefix in the same directory", async () => {
    const tmpDir = makeTmpDir();
  ...
  it("does not group files from different directories", async () => {
    const tmpDir = makeTmpDir();
    touchFile(path.join(tmpDir, "book1", "chapter01.mp3"));
  ...
  it("falls back to directory name when stem patterns don't match", async () => {
    const tmpDir = makeTmpDir();
  ...
  it("still groups by stem when prefix patterns match", async () => {
    const tmpDir = makeTmpDir();
    touchFile(path.join(tmpDir, "The Hobbit - 01.mp3"));
    touchFile(path.join(tmpDir, "The Hobbit - 02.mp3"));
